...
class ClientProcess extends Process_1.Process {
     ...
     * @param options
     */
    onPeerConnection(peerId, options) {
        const length = this.systemNames.length;
        for (let i = 0; i < length; i++) {
            const system = this.systems[this.systemNames[i]];
     ...
     * @param options
     */
    onPeerConnectionRequested(peerId, systemName, options) {
     ...
     * @param options
     */
    onPeerConnectionAccepted(peerId, systemName, options) {
        const system = this.systems[systemName];
     ...
     * @param options
     */
    onPeerConnectionRejected(peerId, systemName) {
        const system = this.systems[systemName];
