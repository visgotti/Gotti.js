...
class Client {
    constructor(url, gameProcessSetups, disableWebRTC = false) {
    ...
    authenticate(options, tokenHeader) {
        return new Promise((resolve, reject) => {
            httpPostAsync(`${this.hostname}${Protocol_1.GOTTI_HTTP_ROUTES.BASE_AUTH}${Protocol_1.GOTTI_HTTP_ROUTES.AUTHENTICATE}`, tokenHeader, { [Protocol_1.GOTTI_AUTH_KEY]: options }, (err, data) => {
                if (err) {
    ...
    register(options, tokenHeader) {
        return new Promise((resolve, reject) => {
            httpPostAsync(`${this.hostname}${Protocol_1.GOTTI_HTTP_ROUTES.BASE_AUTH}${Protocol_1.GOTTI_HTTP_ROUTES.REGISTER}`, tokenHeader, { [Protocol_1.GOTTI_AUTH_KEY]: options }, (err, data) => {
                if (err) {
                    return reject(`Error requesting game ${err}`);
                }
                else {
    ...
    getGames(clientOptions, token) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => {
                httpPostAsync(`${this.hostname}${Protocol_1.GOTTI_HTTP_ROUTES.BASE_GATE}${Protocol_1.GOTTI_HTTP_ROUTES.GET_GAMES}`, token, {
    ...
    joinGame(gameType, joinOptions, token, fps = 60) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return new Promise((resolve, reject) => {
                httpPostAsync(`${this.hostname}${Protocol_1.GOTTI_HTTP_ROUTES.BASE_GATE}${Protocol_1.GOTTI_HTTP_ROUTES.JOIN_GAME}`, token, {
                    gameType,
                    ...
                    if (err) {
                        ...
                        const { gottiId, clientId, host, port, areaOptions } = data;
                        this.runningProcess = yield this.processManager.initializeGame(gameType, areaOptions);
                        const gameData = yield this.joinConnector(gottiId, clientId, `${host}:${port}`);
