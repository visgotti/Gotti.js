...
const ConnectorServerConnections_1 = require("./ConnectorServerConnections");
...
class Connector {
    ...
    connect(connectorAuth, process, processManager, areaData, options = {}, webSocketProtocol) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.serverConnection = new ConnectorServerConnections_1.WebSocketConnection(url);
    ...
    handlePeerServerConnectionRequest(sdpString) {
        this.isTryingToChangeToWebRTC = true;
        this.serverPeerConnection = new PeerConnection_1.PeerConnection(this, this.playerIndex, -1);
    ...
    finalizeWebSocketToWebRTCSwitch() {
        this.isSwitchingGracefullyToWebRTC = true;
        this.serverConnection.close();
        const oldEnqueued = this.serverConnection.enqueuedMessages;
        this.serverConnection = new ConnectorServerConnections_1.WebRTCConnection(this.serverPeerConnection);
        if (oldEnqueued) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (!(this.peerConnections[peerIndex])) {
                ...
                if (!this.pendingPeerRequests[peerIndex]) {
                    const response = yield this.process.onPeerConnectionRequest(peerIndex, systemName, incomingRequestOptions);
                    if (!response) { // our system requester invalidated the connection.
                        this.serverConnection.send([113 /* SIGNAL_FAILED */, peerIndex], true);
                        return;
                    }
                    this.peerConnections[peerIndex] = new PeerConnection_1.PeerConnection(this, this.playerIndex, peerIndex);
    ...
    handleSignalData(peer, signalData) {
    ...
    requestPeerConnection(peerIndex, systemName, requestOptions, systemRequestCallback, ackTimeout, connectionTimeout) {
        ...
        if (!(peerConnection)) {
            peerConnection = new PeerConnection_1.PeerConnection(this, this.playerIndex, peerIndex);
    ...
    joinInitialArea(options) {
        ...
        this.serverConnection.send([20 /* GET_INITIAL_CLIENT_AREA_WRITE */, options], true);
    }
    disconnect() {
    ...
    sendSystemMessage(message, reliable = true) {
        this.serverConnection.send([0 /* SYSTEM_MESSAGE */, message.type, message.data, message.to], reliable);
    }
    sendImmediateSystemMessage(message, reliable = true) {
        this.serverConnection.send([2 /* IMMEDIATE_SYSTEM_MESSAGE */, message.type, message.data, message.to], reliable);
    }
    get hasJoined() {
    ...
    onMessageCallback(message) {
        ...
        if (code === 15 /* JOIN_CONNECTOR */) {
            // [joinOptions]
            this.onJoin(message[1]);
        }
        else if (code === 16 /* JOIN_CONNECTOR_ERROR */) {
        ...
        else if (code === 0 /* SYSTEM_MESSAGE */ || code === 6 /* SYSTEM_MESSAGE_RELIABLE */) {
            this.messageQueue.addRemote(message[1], message[2], message[3], message[4]);
        }
        else if (code === 2 /* IMMEDIATE_SYSTEM_MESSAGE */ || code === 8 /* IMMEDIATE_SYSTEM_MESSAGE_RELIABLE */) {
        ...
        else if (code === 18 /* INITIATE_CHANGE_TO_WEBRTC */) {
            this.handlePeerServerConnectionRequest(message[1]);
        }
        else if (code === 19 /* SERVER_WEBRTC_CANDIDATE */) {
