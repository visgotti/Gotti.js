...
class PeerConnection {
    constructor(connector, clientPlayerIndex, peerPlayerIndex, configOptions) {
        this.doPingInterval = true;
        ...
        this.onAck = new signals_1.Signal();
    ...
    handleLocalDescription(desc) {
        this.peerConnection.setLocalDescription(desc).then(() => {
            ...
            this.connector.serverConnection.send([111 /* SIGNAL_REQUEST */, this.peerPlayerIndex, { sdp: this.peerConnection.localDescription }], true);
        }).catch(err => {
            console.error(err);
        });
    }
    handleInitialLocalDescription(desc, systemName, requestOptions) {
        this.peerConnection.setLocalDescription(desc).then(() => {
            ...
            this.connector.serverConnection.send([114 /* PEER_CONNECTION_REQUEST */, this.peerPlayerIndex, { sdp: this.peerConnection.localDescription }, systemName, requestOptions], true);
        }).catch(err => {
            console.error(err);
        });
    }
    ...
    onIceCandidate(event) {
        ...
        this.connector.serverConnection.send([111 /* SIGNAL_REQUEST */, this.peerPlayerIndex, { candidate }], true);
        return true;
    }
