...
class PeerConnection {
    ...
    startPinging() {
        ...
        this.dataChannel.send(msgpack.encode([Protocol_1.PEER_TO_PEER_PROTOCOLS.PING, this.seq]));
    ...
    handlePong(seq) {
        ...
        if (seq === this.seq) { // we good they sent back our seq
            this.seq++;
            console.log('this.seq was', this.seq);
            if (this.seq === 10) {
                this.destroy();
            }
    ...
    onPeerMessage(event) {
        ...
        if (decoded.length < 3) {
            if (decoded[0] === Protocol_1.PEER_TO_PEER_PROTOCOLS.PING) {
                this.dataChannel.send(msgpack.encode([Protocol_1.PEER_TO_PEER_PROTOCOLS.PONG, decoded[1]]));
            }
